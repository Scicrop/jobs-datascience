...
def naive_classifiers(df_train, df_test, target = None, method=None):
    ...
    for s in strategies: 
        if s =='constant': 
            ...
            recall_test = recall_score(df_test[target], y_pred_test, scoring=method)
            precision_test = precision_score(df_test[target], y_pred_test, scoring=method)
            f1_test = f1_score(df_test[target], y_pred_test, scoring=method)
            recall_train = recall_score(df_train[target], y_pred_train, scoring=method)
            precision_train = precision_score(df_train[target], y_pred_train, scoring=method)
            f1_train = f1_score(df_train[target], y_pred_train, scoring=method)
            ...
            recall_train = recall_score(df_train[target], y_pred_train, scoring=method)
            precision_train = precision_score(df_train[target], y_pred_train, scoring=method)
            f1_train = f1_score(df_train[target], y_pred_train, method)
